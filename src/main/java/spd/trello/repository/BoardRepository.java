...
public class BoardRepository implements IRepository<Board> {
    ...
    public Board create(UUID parent, Board board) {
        try (
                ...
                PreparedStatement ps = connection.prepareStatement("INSERT INTO Board (id,workspace_id, updated_by, created_by, created_date, updated_date, name, description, archived, visibility) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);")) {
            ...
            ps.setBoolean(9, board.getArchived());
    ...
    public List<Board> getAll() {
        ...
        try (
                Connection connection = ConnectionPool.get().getConnection();
                PreparedStatement ps = connection.prepareStatement("SELECT * FROM board")) {
